...
if st.session_state.user_id:
    ...
    # ========== PAGE DONNÉES ==========
        ...
        # (Le code de la page Données est très long, je le conserve mais attention à l'indentation)
    ...
    # ========== PAGE PRÉVISION ==========
        ...
        if proba_finale > 0.6:
            st.error("Actions recommandées : Réduisez immédiatement l'usage des appareils puissants, préparez un groupe électrogène.")
        elif proba_finale > 0.3:
            st.warning("Précautions : Surveillez votre consommation chaque heure, évitez de lancer plusieurs gros appareils en même temps.")
        else:
    ...
    # ========== PAGE RAPPORT ==========
        ...
        scenario_text = f"- **Scénario 1 – Maintien tendance actuelle** : risque autour de {risk_percent:.1f}%.\n- **Scénario 2 – Maintenance préventive** : réduction du risque.\n- **Scénario 3 – Capacités solaires/batteries** : baisse du risque de surcharge."
        st.write(scenario_text)
        st.subheader("🛠️ 6. Décision d'ingénierie recommandée")
        decision_text = f"Intervention ciblée sur le facteur **{dominant}** est prioritaire.\n"
        if dominant == "Surcharge":
            decision_text += "- Installer une solution de peak shaving (solaire + batteries).\n- Renforcer la capacité du transformateur."
        elif dominant == "Fiabilité":
            decision_text += "- Mettre en place une maintenance préventive systématique.\n- Installer une redondance (UPS, groupes électrogènes)."
        else:
            decision_text += "- Améliorer la ventilation des locaux techniques.\n- Surveiller les prévisions météo."
    ...
    # ========== PAGE SOLUTIONS (avec Gemini) ==========
        ...
        if target_energy_kwh <= 0:
            ...
            st.success(f"🏆 **Solution la plus rentable** : {meilleure['Solution']} avec une VAN de {meilleure['VAN (DZD)']:,} DZD et un ROI de {meilleure['ROI 
